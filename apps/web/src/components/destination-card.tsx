  ...
  return (
    <Link href={`/explore/${citizenshipCode}/${destination.code}`} className={linkCardClass}>
      <div className="flex items-start justify-between gap-3">
        <h3 className="text-lg font-semibold text-[var(--foreground)]">{destination.name}</h3>
        <span className="shrink-0 text-xs font-medium uppercase tracking-wide text-[var(--muted)]">
          {routeCountLabel}
        </span>
      </div>
